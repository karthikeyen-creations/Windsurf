...
class SQLiteDAO:
    ...
    def store_queries(self, queries, query_type, source):
        ...
        for i, (table_name, column_value_pairs, line_number) in enumerate(queries, start=1):
            ...
            # logging.info(f"Storing query {query_id} with table name {table_name} and line number {line_number}")
    ...
    def fetch_query_details(self, query_id, tbl_nm):
        ...
        cursor.execute('SELECT id, line_number, table_name, column_name, value, source FROM query_data WHERE id = ?', (query_id,))
        logging.info(f"Query details: {cursor.fetchall()}")
        cursor.execute('''
            SELECT 
